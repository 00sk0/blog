...
[^cf]: 個人的感触で大成功しても+30や+10程度だが失敗すると普通に-60や-90くらい落ちるので．
...
obeliskとclueの対応をすべて列挙すると間に合わないが，最初のobeliskと対応するclueだけ決めればよい．するとtreasureの座標が決まるため，残りの各obeliskについてその座標をtreasureの座標から引くことで対応するclueを求めることが可能．clueをMapに入れて検索することで$O(n^2 logn)$で求解可能．
...
$k$が$n$の約数であるときは1周で終わり，通る点は$1+0k,1+1k,1+2k,...,1+(m-1)k$となる．ただし$m$は$1+mk=n+1$，すなわちちょうど一周するときの値．これらの合計は$\sum_{i=0}^{n/k-1} (1+ik)=\frac{n(n+2-k)} {2k}$となる（数学が出来ないのでWolframで計算した）．

[^hoge]: 証明できているか分からない証明：点$1$に戻るまでの移動距離は$n$の倍数かつ$k$の倍数だから最短でも$nk$．このとき$\frac{nk}{k}=n$点通るが，先の話はどの点にも言えるので$n$点の間に同じ点は$1$以外無い．よってすべての点を通る．
